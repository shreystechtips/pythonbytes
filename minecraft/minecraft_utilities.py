...
# Creates a line segment drawing block type from a list
def LineSegmentBTList(mc, a, b, block):
    if a == b:
        mc.setBlock(a.x, a.y, a.z, block[0][0], block[0][1])
    else:
...
#   block type
# 
# Example arguments of interest:
#   3, True, 30.0, 300.0, 10.0, False, 60.0, 30.0, diamond
#   3, True, 30.0, 300.0, 10.0, False, 55.0, 35.0, diamond
#   3, True, 10.0, 160.0, 6.0, False, 65.0, 25.0, diamond
...
# Platfrom creates a 1-brick horizontal rectangular surface emanating +x +z from a given position
def Platform(mc, pos, x0, y0, z0, xSize, zSize, block, blockQ = 0):
    for i in range(xSize):
        for k in range(zSize):
            mc.setBlock(pos.x + i, pos.y, pos.z + k, block, blockQ)

# ZWall should do what Platform does
# XWall should do what ZWall does

# Shell is a spherical shell centered at pos with radius r
#   It calls the more general SparseShell() to accomplish its task using step = 1
def Shell(mc, pos, r, block, blockQ = 0):
    SparseShell(mc, pos, r, step = 1, block, blockQ)

# SparseShell creates a spherical shell using the Pythagorean formula.
#   It is inefficient because it steps through a cubical volume checking whether locations are 'in' the shell.
#   It does only call setBlock() for correct locations.
def SparseShell(mc, pos, r, step, block, blockQ):
    minR = r - 1
    maxR = r
    for x in range(int(pos.x-r-1), int(pos.x+r+2), step):
        for y in range(int(pos.y-r-1), int(pos.y+r+2), step):
            for z in range(int(pos.z-r-1), int(pos.z+r+2), step):
